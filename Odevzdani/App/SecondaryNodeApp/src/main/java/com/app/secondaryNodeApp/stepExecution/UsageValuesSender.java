    ...
    public String[] createUsageCmd() throws NoSuchFieldException, IllegalArgumentException, IllegalAccessException, IOException {
        ...
        synchronized(stepRuns) {
            for (Process p : stepRuns) {
                if (System.getProperty("os.name").contains("Windows")) {
                    PIDs += Long.toString(getProcessID(p));
                } else {
                    PIDs += getChildPID(p);     // the child PID must be obtained, because stepRuns are /bin/sh processes
                }
        ...
        usageCmd[usageCmd.length - 1] = MessageFormat.format(usageCmd[usageCmd.length - 1], Configuration.getInstance().getPathToBashConfigFolder(), PIDs);
        return usageCmd;
    }
